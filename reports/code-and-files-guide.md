...
- H2：⚠️ **2026-09-21 起判定链路已闭合**（`12ece9d`）—— 逐样本判对错与配对身份键
  随该行落盘，配对 95% CI 算得出来，配对前先逐位比对两份键。仍记 `no-judge`，
  但**唯一剩下的是参数取值**：非劣边界与噪声底线按设计无默认值，需实验者显式声明
  （缺口沿革：定义缺失 → 聚合未定 → 参数待测 → 质量侧通路未独立 → 逐样本未落盘
  → **参数取值**）；
- H3 待 A3 的 GPU 通路；
- H5 待"设备数翻倍"基准点的定义。
