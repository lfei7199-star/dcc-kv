#!/usr/bin/env python
"""E5：消融实验 A1 / A2 / A3 / A5（外加 A4：设计已定、代码待实现）—— GPU 端（需要多卡 + NCCL）。
...
------------------
    ...
    A4  压缩 × 异步交互 —— 预算 × 同步模式的二维格。
        **设计已定（论文 §6.3），代码尚未实现**：不在下方的消融注册表中。
...
**第四，A4 有设计、无代码（编号缺口已显式登记）。**
原件（第 1--4 章）通篇没有「消融」字样，也没有 A1--A5 编号 —— 编号是本仓库引入的。
2026-09-16 之前全仓缺失 A4，编号从 A3 直跳 A5（由独立监督查出，见 `commit_log.md` §S.3）。
现已在论文 §6.3 给出 A4 的设计（预算 × 同步模式二维格 + 两条可证伪预测 + 判定方式），
但**本脚本尚未实现它**，消融注册表因而仍是 A1/A2/A3/A5。此处以文字登记而非静默省略：
读到本文件的人不应把注册表当成编号已经完整。实现时机与 A3 相同 —— 需要
"CompactKV → attention kernel" 的通路可用，且四段计时都能真实测出。
