...
这些是**运行代码反查出来的**，不是读代码猜的。记录时均未修改 `src/`，
只在实验侧记录并设计绕过/检验方式。

⚠️ **下表是历史记录（`5b5ce98` 之前的状态）。第 1–5 条现均已在 `src/` 侧修复**，
保留在此用于溯源。
...
`_env.probe_gpu_construction()` 在目标机上实测 —— 不要在文档里把"源码里不再有
无 device 的张量构造"写成"GPU 构造链路已验证可用"。
...
（历史记录）论文 §6.4 的 E6 原写「3 模型 × **5** 上下文长度 × 2 GPU 数
× 2 同步模式 × 3 基线 = 144 个数据点」，算不出来：`3×5×2×2×3 = 180`；
反推 `144 / (3×2×2×3) = 4`，即长度应为 4 档。`72af7db` 已把论文改为 4 档，
现为 `3×4×2×2×3 = 144`，自洽。

留这段是为了溯源，**不要**再把它当作当前状态引用：`e6_main_table.py --plan`
现在打印的是"按实参现算的数据点数 + 这段历史"，不再复述过期结论。
