...
- [ ] **主表完成**（Table 1 / Table 2）：
  - 3 模型 × 4 长度 × 2 GPU × 2 sync × 3 基线 = 144 数据点
    （原写「5 长度」，与 144 不自洽：3×5×2×2×3 = 180。论文 §6.4 已由
    `72af7db` 改为 4 档，本清单是该修正的漏网副本）
  - 每个数据点 ≥ 10 次 run
...
- [ ] H2：相对共享压缩的质量提升 ≥ 1.5 pp，**且**在质量相近前提下 prefill
      ...
      > **「质量相近」的定义已给出**（2026-09-16）：参照物取**精确注意力**（取共享
      > 压缩基线与「>= 1.5 pp」自相矛盾）；判据为**单侧非劣检验**
      > `CI_low(Q_DCC - Q_dense) > -delta`；非劣边界满足
      > `噪声底线 <= delta < min(1.5, delta_bad)`（`delta_bad` = 共享压缩相对精确
      > 注意力的退化量）；质量侧与性能侧须取自同一组 run；4 个上下文长度分别判定、
      > 全部通过才算前提成立。完整论证见论文 §7「「质量相近」的判定」，机器可读
      > 实现见 `experiments/common/hypotheses.py`。`delta` 与噪声底线依赖尚未取得
      > 的测量，故 H2 仍为 `no-judge` —— 缺口已由「定义缺失」降为「参数待测」。
      >
      > ⚠️ **2026-09-21 更新（`12ece9d`）**：判定**链路**已完整闭合 —— E6 把逐样本
      > 判对错与配对身份键随该行落盘，配对 95% CI 算得出来，且配对前先逐位比对
      > 两份键（不同源即拒绝配对）。H2 仍记 `no-judge`，但**唯一剩下的原因是参数
      > 取值**（`--h2-delta-pp` / `--h2-noise-floor-pp`，刻意无默认值）。
      > **这不是「结论未定」**：链路、口径、阈值都已就位，缺的是一次带参数的运行。
      > 另注：该前提的 CI 方向是 `−ci_95_upper`（不是 `ci_95_lower`），
      > 由 `test_adversarial_2026_09_20.py::test_j2_*` 定向守住。
