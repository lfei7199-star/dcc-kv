...
>
> 生成时间：2026-09-15 01:40 (GMT+8)
> 当前 HEAD：`b8924c2`（分支 `paper/sections-5-8`，连同本次日志提交一并推送；
> 本文件本次补记为紧随其后的 docs 提交 —— 自 `main`（`8a1d275`）分叉以来的第 27 次提交）
>
...
| | | | | **合计** | **175 次文件变更** | **21281** | **699** |

仓库当前规模：97 个受版本控制文件，其中 60 个 `.py`、10 个 `.tex`、15 个 `.md`。

分支与推送状态：

```
* paper/sections-5-8   →  origin/paper/sections-5-8   （907d356..93d4a19 共 25 个提交已推送；
                                                        `b8924c2` 连同本次日志提交待推送）
  main                 →  origin/main                  （未动）
```
...
用户在 2026-09-15 直接问过这句话。诚实的答案是**不能保证**，而且**这一轮的工作本身
就是证据**：就在上一轮宣布"GPU 侧已审完"之后，本轮又查出两处此前从未记录的问题 ——
...
2. 动 `experiments/gpu/` 之前，先让 `tests/test_gpu_pipeline.py`（121 项纯 CPU 锚点）
   与 `tests/test_hypotheses.py`（17 项阈值锚点）通过 —— 它们是当前唯一能在无 GPU
   环境下守住 GPU 侧口径的东西；
