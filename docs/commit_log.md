...
>
> 生成时间：2026-09-14 16:50 (GMT+8)
> 当前 HEAD：`5a3f255`（分支 `paper/sections-5-8`，**有 9 次提交未推送**）
...
| | | | | **合计** | **115 次文件变更** | **17646** | **269** |

仓库当前规模：90 个受版本控制文件，其中 55 个 `.py`、9 个 `.tex`。

分支与推送状态：

```
* paper/sections-5-8   →  origin/paper/sections-5-8
                           (落后 9 个提交：5b5ce98, 511ca1e, e95927f, 8536bbb,
...
### P.1 已在 `5b5ce98` / `511ca1e` / `539ed8c` / `1a4fb06` 中解决
...
| **C9** β 的稀疏塌缩 | 设计层面开放问题 | `1a4fb06`（E10）判定根因**不在秩**而在 β 离散分量的容量代价；`539ed8c` 采纳源论文 Appendix C.2 的箱约束 $[-3,3]$ 为默认，clamp 率 $0.054\to0$。**仅剩 $\lambda_\beta$ 取值待决（见 D4）** |
...
| 项 | 选项 |
|---|---|
| **D1** 是否把这些提交推送到 `origin/paper/sections-5-8`（当前 **9 次提交未推送**） | 推送 / 继续本地累积 |
| **D2** 是否把 §6 的 E2b/E9/E10 拆成独立的补充材料，而非留在正文 | 留正文（当前）/ 移补充材料 |
| **D3** 剩余 6 处中文标点悬挂导致的 overfull 是否要在投稿前处理 | 处理（需改 `\xeCJKsetup`）/ 接受 |
| **D4** 是否把 $\lambda_\beta$ 由 $10^{-3}$ 提升到 $10^{-2}$（`1a4fb06` 测得输出 $\Delta=-0.0054$、$B_{\text{eff}}/B$ $+0.047$、$V$ 残差 $-0.011$，收益大于箱约束） | 提升（须重跑 E2b/E9 并更新表 1/表 2）/ 维持 $10^{-3}$ |
...
- **β 箱约束设为默认后**，E5–E8 在真实模型上的表现（`539ed8c` 只验证了合成数据与
  测试套件，未在任务级指标上验证过该默认值）
