...
>
> ⚠️ **H2 的口径在本仓库有两处表述**：本表写"提升 ≥ 1.5 个百分点"（质量侧），
> `release_checklist.md` §4 写"prefill 加速 ≥ 1.10×"（性能侧）。二者应是同一假设的
> 两个侧面（质量达标 **且** 性能达标），但**合并方式无法证实**：blueprint v1.1 §3
> 原件不在仓库内（见 `commit_log.md` C11）。投稿前须取回原件核对，不得凭推测定稿。
