...
> ⚠️ **2026-09-21 第四次更新（`12ece9d`）**：**H2 的「质量相近」前提已可判。**
> 第三次更新时它仍记 unresolved，原因是「逐样本数据未落盘」（每格只有一个聚合
> 准确率，配对 95% CI 算不出来）。现已把逐样本判对错与配对身份键随该行落盘，
> 配对 CI 交给 `report.paired_bootstrap`；配对前**先逐位比对两行的键**，不同源即
> 拒绝配对（不产出一个"数值正常、实际无意义"的 CI）。
>
> **这对租卡决策的影响**：H2 的判定不再依赖"本机补代码"，只依赖**一次带参数的
> 运行**（`--h2-delta-pp` / `--h2-noise-floor-pp`，两者刻意无默认值）。但 §0 的
> 结论**不变 —— 现在仍不能开跑**：阻断仍是 **G6**（CUDA 构造可用性实测，唯一
> 「只能上机才知道」的一项），以及单卡模拟不体现逐边条件化的代价与收益。
> **H2 可判 ≠ H2 可测**：本机仍产不出真实模型上的任务级数字。
