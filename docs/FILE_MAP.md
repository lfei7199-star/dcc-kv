...
- **行数列的时效性**：本列在多轮提交后未逐次校准 —— 2026-09-16 实测发现约 20 处偏小（1 至 90 行不等，例如 `_hf.py` 436→468、`e6_main_table.py` 441→499、`tests/test_gpu_pipeline.py` 400+→447、`e0_order_invariance.py` 277→429（第九轮校准））。本次只校准了**本轮真涉及的文件**，其余保留原值以免制造假精确。需要准确值时以
