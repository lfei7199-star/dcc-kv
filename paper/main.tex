...
%
%   若用户已有原始 .tex 源文件，应以原始文件替换第 1--4 章正文，仅保留本文新增
%   的第 5--8 章与 figures/、refs.bib。
%
...
% TODO: 替换为真实作者信息（仓库 README 中仍为占位符 YOUR_USERNAME / author@example.com）
\author{作者姓名}
\affiliation{%
  \institution{作者单位}
  \country{China}}
...
FlashAttention \cite{dao2022flashattention} 等工作降低了单设备注意力的显存访存开销，完整
...
$\widehat{Q}_r = Q_r[\mathcal{S}_r^{Q}, :] \in \mathbb{R}^{M \times d_h}$，而非传输投影
向量。选择复杂度为 $O(L_r d_h d_p + M L_r d_p)$。
...
拟合
\begin{equation}
w^* = \arg\min_{w \ge 0} \left\|G^{s \to r} w - m^{s \to r}\right\|_2^2
      + \lambda_\beta \left\|w - \mathbf{1}\right\|_2^2,\qquad
\beta_j^{s \to r} = \log\!\left(\max\{w_j^*, \eps\}\right).
\end{equation}
...
\begin{equation}
S_b = (m_b, l_b, o_b),\quad
m_b = \max_j g_{b,j},\quad
l_b = \sum_j e^{g_{b,j} - m_b},\quad
o_b = \sum_j e^{g_{b,j} - m_b} v_{b,j}.
\end{equation}
对两个状态 $S_a$ 与 $S_b$，定义归并算子 $\oplus$：
\begin{align}
m_{ab} &= \max(m_a, m_b),\\
...
\ForAll{有效边 $(s,r) \in \mathcal{E}$（并行）}
  \State 用 eq.~(14) 选取 $C_k^{s \to r}$
  \State 用 eq.~(16) 拟合 $\beta^{s \to r}$
  \State 用 eq.~(19) 重构 $C_v^{s \to r}$
  \State 打包 $C^{s \to r} = (C_k^{s \to r}, \beta^{s \to r}, C_v^{s \to r})$
\EndFor
\ForAll{目的设备 $r$（并行）}
  \State 计算本地状态 $S_r$，并提交变长消息的非阻塞 All-to-Allv 收发
  \ForAll{已到达的 $C^{s \to r}$（按任意顺序）}
    \State 计算 $S^{s \to r}$，并用 eq.~(25) 立即归并
  \EndFor
  \State 所有接收完成后，用 eq.~(27) 输出 $O_r$
\EndFor
\end{algorithmic}
\end{algorithm}
